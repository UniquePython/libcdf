...
/**
 * Creates an empty column with the specified name and data type.
 *
 * Success:
 *   Allocates and initializes a new column, duplicates `name`, sets its data
 *   type to `type`, and stores it in `*out`.
 *
 * Failure:
 ...
 */
cdf_bool cdfc_create(const char *name, cdfdt type, cdfc **out);
